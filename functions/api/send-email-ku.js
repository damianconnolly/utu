...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const toEmail = env.CONTACT_EMAIL_TO || 'keepieuppies1@outlook.com';
    ...
    const msResponse = await fetch('https://api.mailersend.com/v1/email', {
      ...
      body: JSON.stringify({
        from: { email: fromEmail, name: 'Keepie Uppies Website enquiry' },
        to: [{ email: toEmail }],
        reply_to: { email: email.trim(), name: fullname.trim() },
        subject: `[Keepie Uppies Website] ${subject.trim()}`,
        text: emailBody,
      }),
    });
